...
<body>
    <?php
        ...
        $resultLoaiSua = $DB->get_list($queryLoaiSua);
        $queryHangSua = "SELECT * FROM `hang_sua`";
        $resultHangSua = $DB->get_list($queryHangSua);
...
<div class="app">
    ...
    <div align="center" style="margin-bottom: 20px">
            <form action="./lab2_10-search.php" method="get">
                <div style="margin-bottom: 10px">
                    Loại sữa: <select name="searchLoaiSua">
                        <?php
                        foreach($resultLoaiSua as $value)
                        {
                            if($value['Ma_loai_sua'] == $_GET['searchLoaiSua'])
                            {
                                 echo "<option selected='selected' value='".$value['Ma_loai_sua']."'>".$value['Ten_loai']."</option>";
                            }
                            else
                            {
                                 echo "<option value='".$value['Ma_loai_sua']."'>".$value['Ten_loai']."</option>";
                            }
                        }
                        ?>                        
                    </select>
                    Loại sữa: <select name="searchHangSua">
                    <?php
                    foreach($resultHangSua as $value)
                    {
                        if($value['Ma_hang_sua'] == $_GET['searchHangSua'])
                        {
                                echo "<option selected='selected' value='".$value['Ma_hang_sua']."'>".$value['Ten_hang_sua']."</option>";
                        }
                        else
                        {
                                echo "<option value='".$value['Ma_hang_sua']."'>".$value['Ten_hang_sua']."</option>";
                        }
                    }
                    ?>                        
                    </select><br>
                </div>
                <b>Tên sữa:</b> <input type="text" style="height: 22px; width: 200px" name="searchTenSua"/>
