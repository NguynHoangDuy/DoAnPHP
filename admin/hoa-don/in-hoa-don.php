...
<head>
    ...
    <title>In Hóa đơn - Blue Tea</title>
...
<body onload="window.print();">
    <?php
        ...
        if(isset($_GET["mahd"]))
        {
            ...
            $query = "SELECT `ma_donhang`, `ten_kh`, `gioi_tinh`, `dia_chi`, `so_dt`, `tg_dat` FROM `don_hang` where `ma_donhang` = '$maHD'" ;
                ...
                if(mysqli_num_rows($result) != 0)
                {
                    ...
                    echo '<div class="detial-info">
                        <div class="info-customer">
                            <p class="customer-name"> <b>Họ và tên: </b>'.$row['ten_kh'].'</p>';
                            echo '<p class="customer-gt"> <b>Giới tính: </b>';
                            if($row['gioi_tinh']==0) echo 'Nữ'; else echo 'Nam';
                            echo '</p>
                            <p class="customer-phone"><b>Số điện thoại: </b>'.$row['so_dt'].'</p>
                            <p class="customer-address"><b>Địa chỉ: </b>'.$row['dia_chi'].'</p>
                        </div>
                        <div class="info-receipt">
                            <p class="receipt-id"><b>Mã hóa đơn: </b> '.$row['ma_donhang'].'</p>
                            <p class="receipt-date"><b>Thời gian: </b>'.$row['tg_dat'].'</p>
                        </div>
                    </div>';
                }
